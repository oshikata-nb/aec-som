...
--■■■■■■
	...
	IS
		BF_BACKORDER_QTY NUMBER;
		AF_BACKORDER_QTY NUMBER;
	BEGIN
		LNG_SYORIF := 0;
		OUT_ERROR := '';
		LNG_QTY := NVL(LNG_QTY,0);
		BF_BACKORDER_QTY := CHK_BACKORDER_QTY(STR_ITEMCODE) ;
		--LNG_AZUDIV := 0;
		--在庫更新
		...
		ELSE
			...
			END IF;
		END IF;
		AF_BACKORDER_QTY := CHK_BACKORDER_QTY(STR_ITEMCODE) ;
		CHK_BACKORDER_QTY_NEGATIVE(BF_BACKORDER_QTY,AF_BACKORDER_QTY) ;
...
-- Add Eto 2011.11.18 
FUNCTION CHK_BACKORDER_QTY(ITEMCD VARCHAR2) RETURN NUMBER IS
	CT NUMBER;
	RT NUMBER;
	BEGIN 
		SELECT count(*) INTO CT FROM ITEM_INVENTORY WHERE ITEM_CD = ITEMCD  ;
		IF CT > 0 THEN
			SELECT NVL(BACKORDER_QTY,0) INTO RT FROM ITEM_INVENTORY WHERE ITEM_CD = ITEMCD  ;
		ELSE
			RT := 0 ;
		END IF;
		RETURN RT;
	END;
PROCEDURE CHK_BACKORDER_QTY_NEGATIVE(BEFORE_QTY IN NUMBER, AFTER_QTY IN NUMBER) IS
	err_msg NVARCHAR2(2000);
	BEGIN
		IF AFTER_QTY < 0 THEN
			err_msg := '品目在庫発注残がマイナスになりました。   ITEMCODE :' || STR_ITEMCODE || '  数量変化 :' || BEFORE_QTY || ' -> ' || AFTER_QTY ||
			'   OPNO :' || LNG_OPNO || ' FUNCDIVISION :' || LNG_FUNCDIVISION || '  ORDERNO_LINE_NO : ' || STR_ODERNO || '_' || LNG_ODERLINENO || 
			'   QTY :' || LNG_QTY || '   SLIPNO :' || STR_SLIPNO ;
			OUTPUT_ERROR_LOG('BACKORDER_QTY_MINUS'
				,STR_INPUTNO
				,null
				,err_msg  );
		END IF;
