CREATE OR REPLACE PROCEDURE      TESTKOUBAI
IS
...
/* INOUT_SOURCE オーバーライド */
	DBMS_OUTPUT.PUT_LINE('登録（再）');
 ZAIKOUKEHARAI.ENTRY_PURCHASE(1,ID,'som',para);
 IF para = 'COMPLETE' THEN NULL;ELSE GOTO P_END;END IF;
SELECT COUNT(*) INTO CT FROM INOUT_SOURCE WHERE ODER_NO = 'TESTDATA001' AND ODER_LINE_NO = 0;
IF CT = 1 THEN 
	NULL;
ELSE
	para :='2ENTRY_PURCHASE(1,TESTDATA001,som,para); not Create INOUT_SOURCE';
	GOTO P_END;
END IF;
SELECT INOUT_QTY,ASSIGN_FLAG INTO O_QTY,O_FLG FROM INOUT_SOURCE WHERE INOUT_SOURCE_NO  = STR_IND; 
IF O_QTY = 100 AND O_FLG = 0 THEN
...
END IF;
/* 　品目在庫　引当 - 実在庫 + */
	DBMS_OUTPUT.PUT_LINE('受入+30');
 ZAIKOUKEHARAI.RECEIVE_PURCHASE(1,ID,'som',para);
 IF para = 'COMPLETE' THEN NULL;ELSE GOTO P_END;END IF;
SELECT COUNT(*) INTO CT FROM INOUT_SOURCE WHERE ODER_NO = 'TESTDATA001' AND ODER_LINE_NO = 0;
IF CT = 1 THEN 
	NULL;
ELSE
	para :='RECEIVE_PURCHASE(1,TESTDATA001,som,para); not Create INOUT_SOURCE';
	GOTO P_END;
END IF;
SELECT INOUT_QTY,ASSIGN_FLAG INTO O_QTY,O_FLG FROM INOUT_SOURCE WHERE INOUT_SOURCE_NO  = STR_IND; 
IF O_QTY = 100-30 AND O_FLG = 1 THEN
	NULL;
ELSE
	para :='RECEIVE_PURCHASE(1,TESTDATA001,som,para); ' || TO_CHAR(O_QTY) || ',' || TO_CHAR(O_FLG);
	GOTO P_END;
END IF;
select NVL(INVENTORY_QTY,0),NVL(BACKORDER_QTY,0),NVL(ASSIGN_QTY,0),NVL(SALES_ASSIGN_QTY,0),NVL(FINISH_QTY,0),NVL(INSPECTION_QTY,0) 
INTO O_INVENTORY,O_BACKORDER,O_ASSIGN_QTY,O_SALES,O_FINISH,O_INSPECTION 
...
;
	DBMS_OUTPUT.PUT_LINE('QTY = ' || TO_CHAR(O_QTY) || ' FLG = ' || TO_CHAR(O_FLG) || ' INV = ' || TO_CHAR(O_INVENTORY) || ' BACK = ' || TO_CHAR(O_BACKORDER));
IF O_INVENTORY = I_INVENTORY+30 AND O_BACKORDER = I_BACKORDER + 100 -30 AND O_ASSIGN_QTY = I_ASSIGN_QTY 
   AND O_SALES = I_SALES AND O_FINISH = I_FINISH AND O_INSPECTION = I_INSPECTION THEN
   NULL;
ELSE
	para :='RECEIVE_PURCHASE(1,TESTDATA001,som,para); ' ||TO_CHAR(O_BACKORDER) || '<>' || TO_CHAR(I_BACKORDER + 100) ;
	GOTO P_END;
END IF;
select COUNT(*) 
INTO CT 
...
;
ELSE
	para :='RECEIVE_PURCHASE(1,TESTDATA001,som,para); ILLIGAL CREATE LOCATION_INV'  ;
	GOTO P_END;
END IF;
IF O_INVENTORY = L_INVENTORY+30 AND O_BACKORDER = L_BACKORDER  AND O_ASSIGN_QTY = L_ASSIGN_QTY 
   AND O_SALES = L_SALES AND O_FINISH = L_FINISH AND O_INSPECTION = L_INSPECTION THEN
   NULL;
ELSE
	para :='RECEIVE_PURCHASE(1,TESTDATA001,som,para); ILLIGAL CREATE LOCATION_INV'  ;
...
;
ELSE
	para :='RECEIVE_PURCHASE(1,TESTDATA001,som,para); ILLIGAL CREATE LOT_INV'  ;
	GOTO P_END;
END IF;
IF O_INVENTORY = LL_INVENTORY +30 AND O_BACKORDER = LL_BACKORDER  AND O_ASSIGN_QTY = LL_ASSIGN_QTY 
   AND O_SALES = LL_SALES AND O_FINISH = LL_FINISH AND O_INSPECTION = LL_INSPECTION THEN
   NULL;
ELSE
	para :='RECEIVE_PURCHASE(1,TESTDATA001,som,para); ILLIGAL CREATE LOT_INV'  ;
...
IF CT = 1 THEN 
	NULL;
ELSE
	para :='2RECEIVE_PURCHASE(1,TESTDATA001,som,para); not Create INOUT_SOURCE';
	GOTO P_END;
END IF;
SELECT INOUT_QTY,ASSIGN_FLAG INTO O_QTY,O_FLG FROM INOUT_SOURCE WHERE INOUT_SOURCE_NO  = STR_IND; 
IF O_QTY = 100-30-30 AND O_FLG = 1 THEN
	NULL;
ELSE
	para :='2RECEIVE_PURCHASE(1,TESTDATA001,som,para); ' || TO_CHAR(O_QTY) || ',' || TO_CHAR(O_FLG);
	GOTO P_END;
END IF;
select NVL(INVENTORY_QTY,0),NVL(BACKORDER_QTY,0),NVL(ASSIGN_QTY,0),NVL(SALES_ASSIGN_QTY,0),NVL(FINISH_QTY,0),NVL(INSPECTION_QTY,0) 
INTO O_INVENTORY,O_BACKORDER,O_ASSIGN_QTY,O_SALES,O_FINISH,O_INSPECTION 
...
;
	DBMS_OUTPUT.PUT_LINE('QTY = ' || TO_CHAR(O_QTY) || ' FLG = ' || TO_CHAR(O_FLG) || ' INV = ' || TO_CHAR(O_INVENTORY) || ' BACK = ' || TO_CHAR(O_BACKORDER));
IF O_INVENTORY = I_INVENTORY+30+30 AND O_BACKORDER = I_BACKORDER + 100 -30-30 AND O_ASSIGN_QTY = I_ASSIGN_QTY 
   AND O_SALES = I_SALES AND O_FINISH = I_FINISH AND O_INSPECTION = I_INSPECTION THEN
   NULL;
ELSE
	para :='2RECEIVE_PURCHASE(1,TESTDATA001,som,para); ' || TO_CHAR(O_BACKORDER) || '<>' || TO_CHAR(I_BACKORDER + 100) ;
	GOTO P_END;
END IF;
select NVL(INVENTORY_QTY,0),NVL(BACKORDER_QTY,0),NVL(ASSIGN_QTY,0),NVL(SALES_ASSIGN_QTY,0),NVL(FINISH_QTY,0),NVL(INSPECTION_QTY,0) 
INTO O_INVENTORY,O_BACKORDER,O_ASSIGN_QTY,O_SALES,O_FINISH,O_INSPECTION 
...
WHERE ITEM_CD = '00000087' AND LOCATION_CD = 'G3'
;
IF O_INVENTORY = L_INVENTORY+30+30 AND O_BACKORDER = L_BACKORDER  AND O_ASSIGN_QTY = L_ASSIGN_QTY 
   AND O_SALES = L_SALES AND O_FINISH = L_FINISH AND O_INSPECTION = L_INSPECTION THEN
   NULL;
ELSE
	para :='2RECEIVE_PURCHASE(1,TESTDATA001,som,para); ILLIGAL CREATE LOCATION_INV'  ;
	GOTO P_END;
END IF;
select NVL(INVENTORY_QTY,0),NVL(BACKORDER_QTY,0),NVL(ASSIGN_QTY,0),NVL(SALES_ASSIGN_QTY,0),NVL(FINISH_QTY,0),NVL(INSPECTION_QTY,0) 
INTO O_INVENTORY,O_BACKORDER,O_ASSIGN_QTY,O_SALES,O_FINISH,O_INSPECTION 
...
WHERE ITEM_CD = '00000087' AND LOCATION_CD = 'G3' AND LOT_NO = '999999'
;
IF O_INVENTORY = LL_INVENTORY +30+30 AND O_BACKORDER = LL_BACKORDER  AND O_ASSIGN_QTY = LL_ASSIGN_QTY 
   AND O_SALES = LL_SALES AND O_FINISH = LL_FINISH AND O_INSPECTION = LL_INSPECTION THEN
   NULL;
ELSE
	para :='2RECEIVE_PURCHASE(1,TESTDATA001,som,para); ILLIGAL CREATE LOT_INV'  ;
	GOTO P_END;
END IF;

/* 　品目在庫　引当 - 実在庫 + */
	DBMS_OUTPUT.PUT_LINE('受入取消+30');
 ZAIKOUKEHARAI.RECEIVE_PURCHASE(2,ID,'som',para);
 IF para = 'COMPLETE' THEN NULL;ELSE GOTO P_END;END IF;
SELECT COUNT(*) INTO CT FROM INOUT_SOURCE WHERE ODER_NO = 'TESTDATA001' AND ODER_LINE_NO = 0;
IF CT = 1 THEN 
	NULL;
ELSE
	para :='RECEIVE_PURCHASE(2,TESTDATA001,som,para); not Create INOUT_SOURCE';
	GOTO P_END;
END IF;
SELECT INOUT_QTY,ASSIGN_FLAG INTO O_QTY,O_FLG FROM INOUT_SOURCE WHERE INOUT_SOURCE_NO  = STR_IND; 
IF O_QTY = 100-30 AND O_FLG = 1 THEN
	NULL;
ELSE
	para :='RECEIVE_PURCHASE(2,TESTDATA001,som,para); ' || TO_CHAR(O_QTY) || ',' || TO_CHAR(O_FLG);
	GOTO P_END;
END IF;
select NVL(INVENTORY_QTY,0),NVL(BACKORDER_QTY,0),NVL(ASSIGN_QTY,0),NVL(SALES_ASSIGN_QTY,0),NVL(FINISH_QTY,0),NVL(INSPECTION_QTY,0) 
INTO O_INVENTORY,O_BACKORDER,O_ASSIGN_QTY,O_SALES,O_FINISH,O_INSPECTION 
...
IF O_INVENTORY = I_INVENTORY+30 AND O_BACKORDER = I_BACKORDER + 100 -30 AND O_ASSIGN_QTY = I_ASSIGN_QTY 
   ...
   NULL;
ELSE
	para :='RECEIVE_PURCHASE(2,TESTDATA001,som,para); ' || TO_CHAR(O_BACKORDER) || '<>' || TO_CHAR(I_BACKORDER + 100) ;
	GOTO P_END;
END IF;
select NVL(INVENTORY_QTY,0),NVL(BACKORDER_QTY,0),NVL(ASSIGN_QTY,0),NVL(SALES_ASSIGN_QTY,0),NVL(FINISH_QTY,0),NVL(INSPECTION_QTY,0) 
INTO O_INVENTORY,O_BACKORDER,O_ASSIGN_QTY,O_SALES,O_FINISH,O_INSPECTION 
...
IF O_INVENTORY = L_INVENTORY+30 AND O_BACKORDER = L_BACKORDER  AND O_ASSIGN_QTY = L_ASSIGN_QTY 
   ...
   NULL;
ELSE
	para :='RECEIVE_PURCHASE(2,TESTDATA001,som,para); ILLIGAL CREATE LOCATION_INV'  ;
	GOTO P_END;
END IF;
select NVL(INVENTORY_QTY,0),NVL(BACKORDER_QTY,0),NVL(ASSIGN_QTY,0),NVL(SALES_ASSIGN_QTY,0),NVL(FINISH_QTY,0),NVL(INSPECTION_QTY,0) 
INTO O_INVENTORY,O_BACKORDER,O_ASSIGN_QTY,O_SALES,O_FINISH,O_INSPECTION 
...
IF O_INVENTORY = LL_INVENTORY +30 AND O_BACKORDER = LL_BACKORDER  AND O_ASSIGN_QTY = LL_ASSIGN_QTY 
   ...
   NULL;
ELSE
	para :='RECEIVE_PURCHASE(2,TESTDATA001,som,para); ILLIGAL CREATE LOT_INV'  ;
	GOTO P_END;
END IF;
...
/* 　品目在庫　引当 - 実在庫 + */
 ...
 ZAIKOUKEHARAI.RECEIVE_PURCHASE(1,ID,'som',para);
 IF para = 'COMPLETE' THEN NULL;ELSE GOTO P_END;END IF;
	DBMS_OUTPUT.PUT_LINE('受入+90');
 ZAIKOUKEHARAI.RECEIVE_PURCHASE(1,ID,'som',para);
 IF para = 'COMPLETE' THEN NULL;ELSE GOTO P_END;END IF;
	DBMS_OUTPUT.PUT_LINE('受入+120');
 ZAIKOUKEHARAI.RECEIVE_PURCHASE(1,ID,'som',para);
 IF para = 'COMPLETE' THEN NULL;ELSE GOTO P_END;END IF;
SELECT COUNT(*) INTO CT FROM INOUT_SOURCE WHERE ODER_NO = 'TESTDATA001' AND ODER_LINE_NO = 0;
IF CT = 1 THEN 
	NULL;
ELSE
	para :='3RECEIVE_PURCHASE(1,TESTDATA001,som,para); not Create INOUT_SOURCE';
	GOTO P_END;
END IF;
SELECT INOUT_QTY,ASSIGN_FLAG INTO O_QTY,O_FLG FROM INOUT_SOURCE WHERE INOUT_SOURCE_NO  = STR_IND; 
IF O_QTY = 100-30-30-30-30 AND O_FLG = 1 THEN
	NULL;
ELSE
	para :='2RECEIVE_PURCHASE(1,TESTDATA001,som,para); ' || TO_CHAR(O_QTY) || ',' || TO_CHAR(O_FLG);
	GOTO P_END;
...
;
	DBMS_OUTPUT.PUT_LINE('QTY = ' || TO_CHAR(O_QTY) || ' FLG = ' || TO_CHAR(O_FLG) || ' INV = ' || TO_CHAR(O_INVENTORY) || ' BACK = ' || TO_CHAR(O_BACKORDER));
IF O_INVENTORY = I_INVENTORY+30+30+30+30 AND O_BACKORDER = I_BACKORDER + 100 -100 AND O_ASSIGN_QTY = I_ASSIGN_QTY 
   AND O_SALES = I_SALES AND O_FINISH = I_FINISH AND O_INSPECTION = I_INSPECTION THEN
   NULL;
ELSE
	para :='2RECEIVE_PURCHASE(1,TESTDATA001,som,para); ' || TO_CHAR(O_BACKORDER) || '<>' || TO_CHAR(I_BACKORDER + 100) ;
...
WHERE ITEM_CD = '00000087' AND LOCATION_CD = 'G3'
;
IF O_INVENTORY = L_INVENTORY+30+30+30+30 AND O_BACKORDER = L_BACKORDER  AND O_ASSIGN_QTY = L_ASSIGN_QTY 
   AND O_SALES = L_SALES AND O_FINISH = L_FINISH AND O_INSPECTION = L_INSPECTION THEN
   NULL;
ELSE
	para :='2RECEIVE_PURCHASE(1,TESTDATA001,som,para); ILLIGAL CREATE LOCATION_INV'  ;
...
WHERE ITEM_CD = '00000087' AND LOCATION_CD = 'G3' AND LOT_NO = '999999'
;
IF O_INVENTORY = LL_INVENTORY +30+30+30+30 AND O_BACKORDER = LL_BACKORDER  AND O_ASSIGN_QTY = LL_ASSIGN_QTY 
   AND O_SALES = LL_SALES AND O_FINISH = LL_FINISH AND O_INSPECTION = LL_INSPECTION THEN
   NULL;
ELSE
	para :='2RECEIVE_PURCHASE(1,TESTDATA001,som,para); ILLIGAL CREATE LOT_INV'  ;
...
IF CT = 1 THEN 
	NULL;
ELSE
	para :='2RECEIVE_PURCHASE(2,TESTDATA001,som,para); not Create INOUT_SOURCE';
	GOTO P_END;
END IF;
SELECT INOUT_QTY,ASSIGN_FLAG INTO O_QTY,O_FLG FROM INOUT_SOURCE WHERE INOUT_SOURCE_NO  = STR_IND; 
IF O_QTY = 100-30-30-30 AND O_FLG = 1 THEN
	NULL;
ELSE
	para :='2RECEIVE_PURCHASE(2,TESTDATA001,som,para); ' || TO_CHAR(O_QTY) || ',' || TO_CHAR(O_FLG);
	GOTO P_END;
END IF;
select NVL(INVENTORY_QTY,0),NVL(BACKORDER_QTY,0),NVL(ASSIGN_QTY,0),NVL(SALES_ASSIGN_QTY,0),NVL(FINISH_QTY,0),NVL(INSPECTION_QTY,0) 
INTO O_INVENTORY,O_BACKORDER,O_ASSIGN_QTY,O_SALES,O_FINISH,O_INSPECTION 
...
;
	DBMS_OUTPUT.PUT_LINE('QTY = ' || TO_CHAR(O_QTY) || ' FLG = ' || TO_CHAR(O_FLG) || ' INV = ' || TO_CHAR(O_INVENTORY) || ' BACK = ' || TO_CHAR(O_BACKORDER));
IF O_INVENTORY = I_INVENTORY+30+30+30 AND O_BACKORDER = I_BACKORDER + 100 -30-30-30 AND O_ASSIGN_QTY = I_ASSIGN_QTY 
   AND O_SALES = I_SALES AND O_FINISH = I_FINISH AND O_INSPECTION = I_INSPECTION THEN
   NULL;
ELSE
	para :='2RECEIVE_PURCHASE(2,TESTDATA001,som,para); ' || TO_CHAR(O_BACKORDER) || '<>' || TO_CHAR(I_BACKORDER + 100) ;
	GOTO P_END;
END IF;
select NVL(INVENTORY_QTY,0),NVL(BACKORDER_QTY,0),NVL(ASSIGN_QTY,0),NVL(SALES_ASSIGN_QTY,0),NVL(FINISH_QTY,0),NVL(INSPECTION_QTY,0) 
INTO O_INVENTORY,O_BACKORDER,O_ASSIGN_QTY,O_SALES,O_FINISH,O_INSPECTION 
...
WHERE ITEM_CD = '00000087' AND LOCATION_CD = 'G3'
;
IF O_INVENTORY = L_INVENTORY+30+30+30 AND O_BACKORDER = L_BACKORDER  AND O_ASSIGN_QTY = L_ASSIGN_QTY 
   AND O_SALES = L_SALES AND O_FINISH = L_FINISH AND O_INSPECTION = L_INSPECTION THEN
   NULL;
ELSE
	para :='2RECEIVE_PURCHASE(2,TESTDATA001,som,para); ILLIGAL CREATE LOCATION_INV'  ;
	GOTO P_END;
END IF;
select NVL(INVENTORY_QTY,0),NVL(BACKORDER_QTY,0),NVL(ASSIGN_QTY,0),NVL(SALES_ASSIGN_QTY,0),NVL(FINISH_QTY,0),NVL(INSPECTION_QTY,0) 
INTO O_INVENTORY,O_BACKORDER,O_ASSIGN_QTY,O_SALES,O_FINISH,O_INSPECTION 
...
WHERE ITEM_CD = '00000087' AND LOCATION_CD = 'G3' AND LOT_NO = '999999'
;
IF O_INVENTORY = LL_INVENTORY +30+30+30 AND O_BACKORDER = LL_BACKORDER  AND O_ASSIGN_QTY = LL_ASSIGN_QTY 
   AND O_SALES = LL_SALES AND O_FINISH = LL_FINISH AND O_INSPECTION = LL_INSPECTION THEN
   NULL;
ELSE
	para :='2RECEIVE_PURCHASE(2,TESTDATA001,som,para); ILLIGAL CREATE LOT_INV'  ;
	GOTO P_END;
END IF;
/* 　品目在庫　引当 - 実在庫 + */
	DBMS_OUTPUT.PUT_LINE('受入取消+60');
 ZAIKOUKEHARAI.RECEIVE_PURCHASE(2,ID,'som',para);
 IF para = 'COMPLETE' THEN NULL;ELSE GOTO P_END;END IF;
	DBMS_OUTPUT.PUT_LINE('受入取消+30');
 ZAIKOUKEHARAI.RECEIVE_PURCHASE(2,ID,'som',para);
 IF para = 'COMPLETE' THEN NULL;ELSE GOTO P_END;END IF;
	DBMS_OUTPUT.PUT_LINE('受入取消+0');
 ZAIKOUKEHARAI.RECEIVE_PURCHASE(2,ID,'som',para);
 IF para = 'COMPLETE' THEN NULL;ELSE GOTO P_END;END IF;
SELECT COUNT(*) INTO CT FROM INOUT_SOURCE WHERE ODER_NO = 'TESTDATA001' AND ODER_LINE_NO = 0;
IF CT = 1 THEN 
...
/* 　品目在庫　引当 ゼロ */
 ...
 ZAIKOUKEHARAI.COMPLETE_PURCHASE(5,'TESTDATA001','som',para);
 IF para = 'COMPLETE' THEN NULL;ELSE GOTO P_END;END IF;
SELECT COUNT(*) INTO CT FROM INOUT_SOURCE WHERE ODER_NO = 'TESTDATA001' AND ODER_LINE_NO = 0;
IF CT = 1 THEN 
	NULL;
ELSE
	para :='RECEIVE_PURCHASE(5,TESTDATA001,som,para); not Create INOUT_SOURCE';
	GOTO P_END;
END IF;
SELECT INOUT_QTY,ASSIGN_FLAG INTO O_QTY,O_FLG FROM INOUT_SOURCE WHERE ODER_NO = 'TESTDATA001' AND ODER_LINE_NO = 0;
IF O_QTY = 0 AND O_FLG = 1 THEN
	NULL;
ELSE
	para :='COMPLETE_PURCHASE(TESTDATA001,som,para); ' || TO_CHAR(O_QTY) || ',' || TO_CHAR(O_FLG);
	GOTO P_END;
END IF;
select COUNT(*) 
INTO CT 
...
;
	DBMS_OUTPUT.PUT_LINE('QTY = ' || TO_CHAR(O_QTY) || ' FLG = ' || TO_CHAR(O_FLG) || ' INV = ' || TO_CHAR(O_INVENTORY) || ' BACK = ' || TO_CHAR(O_BACKORDER));
IF O_INVENTORY = I_INVENTORY+30 AND O_BACKORDER = I_BACKORDER AND O_ASSIGN_QTY = I_ASSIGN_QTY 
   AND O_SALES = I_SALES AND O_FINISH = I_FINISH AND O_INSPECTION = I_INSPECTION THEN
   NULL;
ELSE
	para :='COMPLETE_PURCHASE(TESTDATA001,som,para); ' || TO_CHAR(O_BACKORDER) || '<>' || TO_CHAR(I_BACKORDER) ;
	GOTO P_END;
END IF;
ELSE
	para :='CT COMPLETE_PURCHASE(TESTDATA001,som,para); ' || TO_CHAR(O_BACKORDER) || '<>' || TO_CHAR(I_BACKORDER) ;
	GOTO P_END;
END IF;
select COUNT(*) 
INTO CT 
...
IF O_INVENTORY = L_INVENTORY+30 AND O_BACKORDER = L_BACKORDER  AND O_ASSIGN_QTY = L_ASSIGN_QTY 
   ...
   NULL;
ELSE
	para :='COMPLETE_PURCHASE(TESTDATA001,som,para); ILLIGAL CREATE LOCATION_INV'  ;
	GOTO P_END;
END IF;
ELSE
	para :='COMPLETE_PURCHASE(TESTDATA001,som,para); ILLIGAL CREATE LOCATION_INV'  ;
...
IF O_INVENTORY = LL_INVENTORY +30 AND O_BACKORDER = LL_BACKORDER  AND O_ASSIGN_QTY = LL_ASSIGN_QTY 
   ...
   NULL;
ELSE
	para :='COMPLETE_PURCHASE(5,TESTDATA001,som,para); ILLIGAL CREATE LOT_INV'  ;
	GOTO P_END;
END IF;
ELSE
	para :='COMPLETE_PURCHASE(5,TESTDATA001,som,para); ILLIGAL CREATE LOT_INV'  ;
