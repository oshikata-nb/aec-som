 ...
 * 受注取込基データヘッダ用SQL 
 *
 * entityName=OrderImportDataEntity
 * packageName=orderimportdataentity
 * methodName=getImportDataEntity
...
WITH TARGET_DATA AS (
    ...
    ,   CASE WHEN ORDER_IMP_BASE.ORDER_IMP_NO IS NOT NULL AND ORDER_VENDER_DELIVERY.SOM_DELIVERY_CD IS NULL THEN '変換マスタに客先納入先コードを登録してください。'  END AS ERROR_MSG1
    ,   CASE WHEN ORDER_IMP_BASE.ORDER_IMP_NO IS NOT NULL AND DELIVERY.DELIVERY_CD IS NULL THEN '変換マスタに登録されている納入先を納入先マスタに登録してください。' END AS ERROR_MSG2
    ,   CASE WHEN ORDER_IMP_BASE.ORDER_IMP_NO IS NOT NULL AND FRST_ORDER_HEAD.BALANCE_CD IS NULL THEN '最上位の得意先を取得できませんでした。帳合マスタを設定してください。' END AS ERROR_MSG3
    FROM 
        FRST_ORDER_HEAD
    INNER JOIN
        FRST_ORDER_DETAIL
