 ...
 * methodName=getCommonAttributeQueueListForReport
 *
 */
...
, ITEM
, COMMON_ATTRIBUTE_QUEUE COMMON
, LOGIN INPUTOR
, LOGIN UPDATOR

WHERE ITEM_QUEUE.ITEM_CD = COMMON.ITEM_CD
AND ITEM_QUEUE.VERSION = COMMON.VERSION
AND COMMON.INPUTOR_CD = INPUTOR.TANTO_CD(+)
AND COMMON.UPDATOR_CD = UPDATOR.TANTO_CD(+)
AND ITEM_QUEUE.ITEM_CD = ITEM.ITEM_CD(+)
AND ITEM_QUEUE.VERSION = ITEM.VERSION(+)

ORDER BY COMMON.ITEM_CD, DECODE(ITEM_QUEUE.VERSION, ITEM.VERSION, 1, 0) DESC, ITEM_QUEUE.ACTIVE_DATE DESC, COMMON.VERSION
