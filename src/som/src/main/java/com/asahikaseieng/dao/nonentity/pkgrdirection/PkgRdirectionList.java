...
import com.asahikaseieng.app.comboboxes.SelectPkgRdirectionDirectionStatus;
 ...
 * @author tosco
 */
public class PkgRdirectionList extends PkgRdirectionListBase implements
	...
	private void initLabelCount() {
		...
		if (getPaletteProducts() != null && !getPaletteProducts().equals(BigDecimal.ZERO)) {

			if (this.getPlanedQty() != null) {
				// 予定生産量 / パレット上製品数
				setLabelCount(
					getPlanedQty().divide(getPaletteProducts()
						, 0
	...
	public String getStatusName() {
		...
		if (directionStatus != null) {
			statusName = SelectPkgRdirectionDirectionStatus.getLabelName(directionStatus.toString());
		}
		return statusName;
	}
	 ...
	 * @return 包装開始予定日時
	 */
	public String getPackResultSdate() {
		String strDate = "";
		if (this.getResultSdate() != null) {
			strDate = AecDateUtils.dateFormat(this.getResultSdate(), "yyyy/MM/dd HH:mm");
		}
		return strDate;
	}
	 ...
	 * @return 包装終了予定日時
	 */
	public String getPackResultEdate() {
		String strDate = "";
		if (this.getResultSdate() != null) {
			strDate = AecDateUtils.dateFormat(this.getResultEdate(), "yyyy/MM/dd HH:mm");
		}
		return strDate;
	}
