 ...
 * @author tosco
 */
public class ShippingResultLotSearchList extends ShippingResultLotSearchListBase implements
	 ...
	 * コンストラクタ.
	 */
	public ShippingResultLotSearchList() {
		super();
	}
	/* ---------- callbacker ---------- */
	...
	public void init() throws Exception {
		...
		//有効在庫算出
		setValidInventory(calcValidInventory(getInventoryQty(), getBackorderQty(),
			getInspectionQty(), getAssignQty()));
	 ...
	 */
	private BigDecimal calcValidInventory(final BigDecimal inventoryQty, final BigDecimal backorderQty,
			final BigDecimal inspectionQty, final BigDecimal assignQty) {
		BigDecimal validInventory = new BigDecimal(0);

		if (inventoryQty == null || backorderQty == null || inspectionQty == null || assignQty == null) {
			return null;
		}

		// 有効在庫 ＝ 在庫量 ＋ 発注残 ＋ 検査待 － 引当残
		validInventory = inventoryQty.add(backorderQty).add(inspectionQty).add(assignQty);
		return validInventory;
	}
