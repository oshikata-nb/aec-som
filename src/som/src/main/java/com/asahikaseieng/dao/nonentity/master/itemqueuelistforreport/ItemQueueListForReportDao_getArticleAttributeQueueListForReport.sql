/*
 * 品目マスタ在庫単価販売品検索帳票用SQL
 *
 * entityName=ItemQueueListForReport
 * packageName=itemqueuelistforreport
 * methodName=getArticleAttributeQueueListForReport
 *
 */
...
, FINANCIAL.FINANCIAL_CLASS_NAME
...
WHERE ITEM_QUEUE.ITEM_CD = ARTICLE.ITEM_CD
AND ITEM_QUEUE.VERSION = ARTICLE.VERSION
AND ARTICLE.SECTION_CD = BUMON.SECTION_CD(+)
AND ARTICLE.ACCOUNTS_CD = ACCOUNTS.ACCOUNTS_CD(+)
AND ARTICLE.FINANCIAL_CLASS_CD = FINANCIAL.FINANCIAL_CLASS_CD(+)
AND ARTICLE.INPUTOR_CD = INPUTOR.TANTO_CD(+)
AND ARTICLE.UPDATOR_CD = UPDATOR.TANTO_CD(+)
AND ITEM_QUEUE.ITEM_CD = ITEM.ITEM_CD(+)
AND ITEM_QUEUE.VERSION = ITEM.VERSION(+)

ORDER BY ARTICLE.ITEM_CD, DECODE(ITEM_QUEUE.VERSION, ITEM.VERSION, 1, 0) DESC, ITEM_QUEUE.ACTIVE_DATE DESC, ARTICLE.VERSION
