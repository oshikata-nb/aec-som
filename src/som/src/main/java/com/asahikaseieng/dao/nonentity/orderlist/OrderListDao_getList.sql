...
,	ORDER_LIST.DIVISION_NAME AS ORDER_DIVISION_NAME
,	ORDER_LIST.DELIVERY_NAME
,	SUBSTRB(ORDER_LIST.DELIVERY_NAME, 0, 100) SHORT_DELIVERY_NAME

,	ORDER_LIST.VENDER_NAME
,	SUBSTRB(ORDER_LIST.VENDER_NAME, 0, 100) SHORT_VENDER_NAME

,	ORDER_LIST.ITEM_NAME
,	SUBSTRB(ORDER_LIST.ITEM_NAME, 0, 100) SHORT_ITEM_NAME
...
--	AND	ORDER_HEAD.ORDER_NO = INPUT_TANTO.ORDER_NO(+)
	...
	AND		ORDER_HEAD.ORDER_NO >= /*condition.srhOrderNoFrom*/'JU00000001' 
	/*END*/
	
	-- 受注番号To
	/*IF (condition.srhOrderNoTo != null) && (condition.srhOrderNoTo != "")*/
	AND 	ORDER_HEAD.ORDER_NO <= /*condition.srhOrderNoTo*/'JU999999999' 
	/*END*/
	
	-- 受注区分が全てではない場合
	/*IF (condition.srhOrderDivision != 0)*/
	AND 	ORDER_HEAD.ORDER_DIVISION = /*condition.srhOrderDivision*/3
	/*END*/
	
	-- 受注日From
	/*IF(condition.srhOrderDateFrom != null) && (condition.srhOrderDateFrom != "")*/
	AND 	TO_CHAR(ORDER_HEAD.ORDER_DATE, 'YYYY/MM/DD') >= /*condition.srhOrderDateFrom*/'2000/01/01'
	/*END*/
	
	-- 受注日To
	/*IF(condition.srhOrderDateTo != null) && (condition.srhOrderDateTo != "")*/
	AND 	TO_CHAR(ORDER_HEAD.ORDER_DATE, 'YYYY/MM/DD') <= /*condition.srhOrderDateTo*/'2010/12/31'
	/*END*/
	
	-- 出荷予定日From
	/*IF(condition.srhScheduledShippingDateFrom != null) && (condition.srhScheduledShippingDateFrom != "")*/
	AND 	TO_CHAR(ORDER_HEAD.SCHEDULED_SHIPPING_DATE, 'YYYY/MM/DD') >= /*condition.srhScheduledShippingDateFrom*/'2000/01/01'
	/*END*/
	
	-- 出荷予定日To
	/*IF(condition.srhScheduledShippingDateTo != null) && (condition.srhScheduledShippingDateTo != "")*/
	AND 	TO_CHAR(ORDER_HEAD.SCHEDULED_SHIPPING_DATE, 'YYYY/MM/DD') <= /*condition.srhScheduledShippingDateTo*/'2010/12/31'
	/*END*/
