/*
 * 販売条件・見積単価　コピー作成・削除帳票用
 *
 * entityName=SalestermsAndEstimateListForReport
 * packageName=salestermsandestimatelistforreport
...
SELECT
	...
	,INPUTOR.TANTO_NM INPUTOR_NAME			--入力者名
	,SALES_ESTI.UPDATE_DATE					--更新日時
	,SALES_ESTI.UPDATOR_CD					--更新者コード
	,UPDATOR.TANTO_NM UPDATOR_NAME			--更新者名
...
LEFT JOIN
	LOGIN　INPUTOR ON SALES_ESTI.INPUTOR_CD =  INPUTOR.TANTO_CD
	
LEFT JOIN
	LOGIN　UPDATOR ON SALES_ESTI.INPUTOR_CD =  UPDATOR.TANTO_CD
...
AND	(SALES_ESTI.INPUTOR_CD LIKE /*condition.srhTantoCd*/'%' OR INPUTOR.TANTO_NM LIKE /*condition.srhTantoCd*/'%')
/*END*/
