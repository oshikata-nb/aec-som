 ...
 * methodName=getComponentInformationQueueListForReport
 *
 */
...
, ITEM
, COMPONENT_INFORMATION_QUEUE INFORMATION
, COMPONENT
, LOGIN INPUTOR
, LOGIN UPDATOR

WHERE ITEM_QUEUE.ITEM_CD = INFORMATION.ITEM_CD
AND ITEM_QUEUE.VERSION = INFORMATION.VERSION
AND INFORMATION.COMPONENT_CD = COMPONENT.COMPONENT_CD(+)
AND INFORMATION.INPUTOR_CD = INPUTOR.TANTO_CD(+)
AND INFORMATION.UPDATOR_CD = UPDATOR.TANTO_CD(+)
AND ITEM_QUEUE.ITEM_CD = ITEM.ITEM_CD(+)
AND ITEM_QUEUE.VERSION = ITEM.VERSION(+)

ORDER BY INFORMATION.ITEM_CD, DECODE(ITEM_QUEUE.VERSION, ITEM.VERSION, 1, 0) DESC, ITEM_QUEUE.ACTIVE_DATE DESC, INFORMATION.VERSION, INFORMATION.INDICATE_ORDER
