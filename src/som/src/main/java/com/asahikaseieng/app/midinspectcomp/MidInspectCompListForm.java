 ...
 * 
 */
public final class MidInspectCompListForm extends AbstractSearchForm {
	...
	/** 帳票Excel検索用 */
	private MidInspectCompListConditionForReport reportCondition;
	...
	/** リスト */
	private List<MidInspectCompList> searchList;

	/**
	 * コンストラクタ
	 */
	public MidInspectCompListForm() {
	}
	...
	protected Class getPagerConditionClass() {
		return MidInspectCompPagerCondition.class;
	}
	...
	public ActionErrors validate(final ActionMapping mapping,
		...
		if ("search".equals(getOp())) {
			...
			setSearchList(new ArrayList<MidInspectCompList>());
			// Validatorによる判定
	...
	private void validateSearchList(final ActionErrors errors) {
		...
		if (!StringUtils.isEmpty(getSrhPlanedSDayFrom())) {
			...
			if (!AecDateUtils.chkDate(strSrhPlanedSdateFrom, strFormat)) {
				errors.add("", new ActionMessage(
						"errors.midinspectcomp.datetime",
						rb.getString("item.midinspectcomp.planed.sdate.from")));
			} else {
			...
			if (!StringUtils.isEmpty(getSrhPlanedSTimeFrom())) {
				errors.add("", new ActionMessage(
						"errors.midinspectcomp.time.notinputday",
						rb.getString("item.midinspectcomp.planed.sdate.from")));
			}
		}
		// 製造開始予定日時(To)
		if (!StringUtils.isEmpty(getSrhPlanedSDayTo())) {
			...
			if (!AecDateUtils.chkDate(strSrhPlanedSdateTo, strFormat)) {
				errors.add("", new ActionMessage(
						"errors.midinspectcomp.datetime",
						rb.getString("item.midinspectcomp.planed.sdate.to")));
			} else {
		...
		if (!StringUtils.isEmpty(getSrhPlanedEDayFrom())) {
			...
			if (!AecDateUtils.chkDate(strSrhPlanedEdateFrom, strFormat)) {
				errors.add("", new ActionMessage(
						"errors.midinspectcomp.datetime",
						rb.getString("item.midinspectcomp.planed.edate.from")));
			} else {
		...
		if (!StringUtils.isEmpty(getSrhPlanedEDayTo())) {
			...
			if (!AecDateUtils.chkDate(strSrhPlanedEdateTo, strFormat)) {
				errors.add("", new ActionMessage(
						"errors.midinspectcomp.datetime",
						rb.getString("item.midinspectcomp.planed.edate.to")));
			} else {
		...
		if (planedSdateFrom != null && planedSdateTo != null) {
			if (planedSdateFrom.compareTo(planedSdateTo) > 0) {
				errors.add("", new ActionMessage(
						"errors.midinspectcomp.datetime.compare",
						rb.getString("item.midinspectcomp.planed.sdate.from"),
						rb.getString("item.midinspectcomp.planed.sdate.to")));
			}
		}
	...
	private void clearCheck() {
		if (getSearchList() != null) {
			for (MidInspectCompList bean : getSearchList()) {
				bean.setMidInspectCompCheckBox(Boolean.FALSE);
			}
		}
	}
	 ...
	 */
	public void setSearchList(final List<MidInspectCompList> searchList) {
	 ...
	 * @return String 検索入力：製造開始予定日(FROM)
	 */
	 ...
	 * @param srhPlanedSDayFrom 検索入力：製造開始予定日(FROM)
	 */
	...
	/**
	 * 検索入力：製造開始予定日(TO)設定
	 * @param srhPlanedSDayTo 検索入力：製造開始予定日(TO)
	 */
	 ...
	 * @return String 検索入力：製造開始予定時(FROM)
	 */
	 ...
	 * @param srhPlanedSTimeFrom 検索入力：製造開始予定日(FROM)
	 */
	...
	/**
	 * 検索入力：製造開始予定時(TO)設定
	 * @param srhPlanedSTimeTo 検索入力：製造開始予定時(TO)
	 */
	 ...
	 * @return String 検索入力：製造終了予定日(FROM)
	 */
	 ...
	 * @param srhPlanedEDayFrom 検索入力：製造終了予定日(FROM)
	 */
	...
	/**
	 * 検索入力：製造終了予定日(TO)設定
	 * @param srhPlanedEDayTo 検索入力：製造終了予定日(TO)
	 */
	 ...
	 * @param srhPlanedETimeFrom 検索入力：製造終了予定時(FROM)
	 */
	...
	/**
	 * 検索入力：製造終了予定時(TO)設定
	 * @param srhPlanedETimeTo 検索入力：製造終了予定時(TO)
	 */
	 ...
	 */
	public MidInspectCompListConditionForReport getReportCondition() {
		return reportCondition;
	}
	...
	public void setReportCondition(
			final MidInspectCompListConditionForReport reportCondition) {
