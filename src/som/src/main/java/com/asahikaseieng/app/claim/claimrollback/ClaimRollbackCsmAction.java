...
/**
 * 請求更新ロールバック処理 Actionクラス.(カスタマイズ)
 * @author tosco
 */
public class ClaimRollbackCsmAction extends AbstractAction {

	/** 取引先区分=TS(得意先) */
	private static final String VENDER_DIVISION_TS = "TS";

	private ClaimRollbackCsmLogic claimRollbackLogic;

	private VenderListLogic venderListLogic;

	/**
	 * コンストラクタ.請求更新ロールバック処理(カスタマイズ)
	 */
	public ClaimRollbackCsmAction() {
		super();
	}
	...
	public ActionForward init(final ActionMapping mapping,
		...
		ClaimRollbackCsmForm frm = (ClaimRollbackCsmForm) form;

		/* 権限取得 */
		getControlAuthority(request, frm, Constants.MENU_ID_CLAIM_ROLLBACK,
			Constants.TAB_ID_CLAIM_ROLLBACK_DETAIL);
	...
	public ActionForward update(final ActionMapping mapping,
		...
		// 部署コード存在チェック
		if (claimRollbackLogic.getOrganization(frm.getOrganizationCd()) == null) {
			saveError(request, "errors.nodata.master", rb
					.getString("item.claimrollback.organization.cd"));
		...
		if (rtn == null || rtn.compareTo(new BigDecimal(0)) != 0) {
			...
			if (dto.getPNumUpdateCount().intValue() == 0) {
				// ロールバック対象なし
				saveError(request, "errors.updaterollback.no.data");
				return mapping.getInputForward();
			}
		}
	 ...
	 */
	private boolean checkRollbackExist(final ClaimRollbackCsmForm frm) {
		boolean res = false;
	 ...
	 * @return boolean 存在結果（true:存在する false:存在しない）
	 */
	// private boolean checkExist(final ClaimRollbackCsmForm frm) {
	// boolean res = false;
	//
	// // 請求ヘッダー検索処理
	 ...
	 * @return boolean 存在結果（true:存在する false:存在しない）
	 */
	// private boolean checkEraserExist(final ClaimRollbackCsmForm frm) {
	// boolean res = false;
	//
	// // 消込データ検索処理
	 ...
	 * @param claimRollbackLogic ClaimCsmRollbackLogic
	 */
	public void setClaimRollbackLogic(
			final ClaimRollbackCsmLogic claimRollbackLogic) {
