 ...
 * @author tosco
 */
public final class GrecipeFormulaListForm extends AbstractGrecipeForm {
	...
	/** 検索結果リスト */
	private List<GrecipeRecipeFormulaList> searchFormList;

	/** 配合検索行番号 */
	private String line;

	/** 行削除リスト */
	private List<GrecipeRecipeFormulaList> delFormList;

	/** 表示時処方ヘッダー情報 */
	private GrecipeRecipeHeaderList headerBean;
	...
	public ActionErrors validate(final ActionMapping mapping,
		...
		if ("search".equals(getOp())) {
			...
			setSearchFormList(new ArrayList<GrecipeRecipeFormulaList>());
			setDelFormList(new ArrayList<GrecipeRecipeFormulaList>());
		}
	 ...
	 * @return 検索結果リスト
	 */
	public List<GrecipeRecipeFormulaList> getSearchFormList() {
	 ...
	 * @return delFormList
	 */
	public List<GrecipeRecipeFormulaList> getDelFormList() {
		return delFormList;
	}
	 ...
	 */
	public void setDelFormList(final List<GrecipeRecipeFormulaList> delFormList) {
	 ...
	 */
	public GrecipeRecipeHeaderList getHeaderBean() {
		return headerBean;
	}
	 ...
	 */
	public void setHeaderBean(final GrecipeRecipeHeaderList headerBean) {
	...
	private void clearCheck() {
		if (getSearchFormList() != null) {
			for (GrecipeRecipeFormulaList bean : getSearchFormList()) {
	...
	private void validateSearchListUPD(final ActionErrors errors,
			final HttpServletRequest request) {
		List<GrecipeRecipeFormulaList> list = this.getSearchFormList();
		int index = 1;
		...
		if (list == null) {
			errors.add("", new ActionMessage("errors.grecipe.no.search"));
		} else {
			...
			for (GrecipeRecipeFormulaList bean : list) {
				...
				if (StringUtils.isEmpty(bean.getItemCd())) {
					errors.add("", new ActionMessage("errors.required.row", rb
							.getString("item.grecipe.product"), Integer
							.toString(index)));
				}
				...
				if (StringUtils.isEmpty(bean.getStrQty())) {
					errors.add("", new ActionMessage("errors.required.row", rb
							.getString("item.grecipe.formula.qty"), Integer
							.toString(index)));
				} else {
					CheckDigitUtilsLogic check = CheckDigitUtil
							.getCheckDigitUtils(request);
					ActionMessage message = check.checkDigitMessage(
						GrecipeConst.UNIT_DIVISION_HAIGO, null, null, bean
								.getStrQty(), rb
								.getString("item.grecipe.formula.qty"));
					if (message != null) {
						errors.add("", message);
					}
				}
			...
			List<GrecipeRecipeFormulaList> delList = this.getDelFormList();
			// 更新の必要なし
