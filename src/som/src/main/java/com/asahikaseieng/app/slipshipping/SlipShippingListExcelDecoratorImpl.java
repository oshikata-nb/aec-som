 ...
 * @author t1344224
 */
public class SlipShippingListExcelDecoratorImpl implements
		SlipShippingListExcelDecorator {
	 ...
	 * コンストラクタ
	 */
	public SlipShippingListExcelDecoratorImpl() {
		super();
	}
	...
	public FileDownloadInfo createReport(
		...
		/* テンプレートをセット */
		builder.setWorkbookUrl("slipshippinglist");
	 ...
	 */
	private void setHeader(final SlipShippingListConditionForReport condition) {
		short rowNum = TEMP_START_ROW;
		short colNum = TEMP_START_COL;

		List<SlipShippingListForReport> list = slipShippingListLogic
				.getSlipHeaderList(condition);
		/* シートをセット */
		builder.setSheet("shipping");

		/* リスト部 */
		for (SlipShippingListForReport bean : list) {
	 ...
	 * @param condition 検索条件
	 */
	private void setDetail(final SlipShippingListConditionForReport condition) {
		short rowNum = TEMP_START_ROW;
		short colNum = TEMP_START_COL;

		List<SlipShippingDetailListForReport> detailList = slipShippingListLogic
				.getSlipDetailList(condition);
		/* シートをセット */
		builder.setSheet("shipping_detail");

		/* リスト部 */
		for (SlipShippingDetailListForReport bean : detailList) {
			colNum = TEMP_START_COL;
			builder.setExcelDataString(rowNum, colNum++, bean.getShippingNo());
			builder.setExcelDataBigDecimal(rowNum, colNum++, bean
					.getShippingRowNo());
