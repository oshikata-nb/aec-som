...
/**
 * 受入・仕入検索画面の購買ステータス用コンボボックスクラス
 * @author tosco
 */
public class SelectPurchaseStatusAccept extends AbstractSelectComboBox {
	...
	private static final String[] myValue = {PurchaseStatus.ISSUED.toString(),
			...
			PurchaseStatus.ACCEPTED.toString()};

	private static final String[] myLabel = {PurchaseStatus.ISSUED_NAME,
			PurchaseStatus.TYOSEI_NAME, PurchaseStatus.FIXED_NAME,
			PurchaseStatus.ARRIVALED_NAME, PurchaseStatus.ACCEPTED_NAME};

	/**
	 * コンストラクタ
	 */
	public SelectPurchaseStatusAccept() {
		setItem();
	}
	 ...
	 */
	public SelectPurchaseStatusAccept(final boolean zero, final boolean withNum) {
		super(zero, withNum);
		setItem();
	}
