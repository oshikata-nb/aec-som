/*
 * Created on 2009/02/01
 *
 * $copyright$
 *
 */
package com.asahikaseieng.app.master.accounts;

import java.math.BigDecimal;

import javax.servlet.http.HttpServletRequest;

import org.apache.struts.action.ActionErrors;
import org.apache.struts.action.ActionMapping;

import com.asahikaseieng.struts.AbstractForm;

/**
 * 勘定科目詳細 Formクラス.
 * @author t0011036
 */
public final class AccountsDetailForm extends AbstractForm {

	private static final long serialVersionUID = 1L;

	/* 勘定科目コード */
	private String accountsCd;

	/* 勘定科目名称 */
	private String accountsName;

	/* 課税区分 */
	private String taxationDivision;

	/* 仕入科目区分 */
	private BigDecimal purchaseAccounts;

	/* 売上科目区分 */
	private BigDecimal articleAccounts;

	/* 更新日時 */
	private java.sql.Timestamp updateDate;

	/* 変更フラグ */
	private Boolean dirtyFlg;

	/* 新規フラグ */
	private String newFlg;

	/**
	 * コンストラクタ.
	 */
	public AccountsDetailForm() {
	}

	/**
	 * {@inheritDoc}
	 */
	public void reset(final ActionMapping mapping,
			final HttpServletRequest request) {
		super.reset(mapping, request);

		if ("init".equals(getOp())) {
			/* クリア処理 */
			clear();
		}
	}

	/**
	 * {@inheritDoc}
	 */
	public ActionErrors validate(final ActionMapping mapping,
			final HttpServletRequest request) {
		ActionErrors errors = null;

		if ("update".equals(getOp())) {
			/* Validatorによる判定 */
			errors = super.validate(mapping, request);
		}

		return errors;
	}

	/**
	 * クリア処理.
	 */
	public void clear() {
		setAccountsCd(null);
		setAccountsName(null);
		setTaxationDivision(null);
		setPurchaseAccounts(null);
		setArticleAccounts(null);
		setUpdateDate(null);
		setDirtyFlg(null);
		setNewFlg(null);
	}

	/**
	 * newFlgを取得します。
	 * @return newFlg
	 */
	public String getNewFlg() {
		return newFlg;
	}

	/**
	 * newFlgを設定します。
	 * @param newFlg newFlg
	 */
	public void setNewFlg(final String newFlg) {
		this.newFlg = newFlg;
	}

	/**
	 * updateDateを取得します。
	 * @return updateDate
	 */
	public java.sql.Timestamp getUpdateDate() {
		return updateDate;
	}

	/**
	 * updateDateを設定します。
	 * @param updateDate updateDate
	 */
	public void setUpdateDate(final java.sql.Timestamp updateDate) {
		this.updateDate = updateDate;
	}

	/**
	 * dirtyFlgを取得します。
	 * @return dirtyFlg
	 */
	public Boolean getDirtyFlg() {
		return dirtyFlg;
	}

	/**
	 * dirtyFlgを設定します。
	 * @param dirtyFlg dirtyFlg
	 */
	public void setDirtyFlg(final Boolean dirtyFlg) {
		this.dirtyFlg = dirtyFlg;
	}

	/**
	 * accountsCdを取得します。
	 * @return accountsCd
	 */
	public String getAccountsCd() {
		return accountsCd;
	}

	/**
	 * accountsCdを設定します。
	 * @param accountsCd accountsCd
	 */
	public void setAccountsCd(final String accountsCd) {
		this.accountsCd = accountsCd;
	}

	/**
	 * accountsNameを取得します。
	 * @return accountsName
	 */
	public String getAccountsName() {
		return accountsName;
	}

	/**
	 * accountsNameを設定します。
	 * @param accountsName accountsName
	 */
	public void setAccountsName(final String accountsName) {
		this.accountsName = accountsName;
	}

	/**
	 * taxationDivisionを取得します。
	 * @return taxationDivision
	 */
	public String getTaxationDivision() {
		return taxationDivision;
	}

	/**
	 * taxationDivisionを設定します。
	 * @param taxationDivision taxationDivision
	 */
	public void setTaxationDivision(final String taxationDivision) {
		this.taxationDivision = taxationDivision;
	}

	/**
	 * articleAccountsを取得します。
	 * @return articleAccounts
	 */
	public BigDecimal getArticleAccounts() {
		return articleAccounts;
	}

	/**
	 * articleAccountsを設定します。
	 * @param articleAccounts articleAccounts
	 */
	public void setArticleAccounts(final BigDecimal articleAccounts) {
		this.articleAccounts = articleAccounts;
	}

	/**
	 * purchaseAccountsを取得します。
	 * @return purchaseAccounts
	 */
	public BigDecimal getPurchaseAccounts() {
		return purchaseAccounts;
	}

	/**
	 * purchaseAccountsを設定します。
	 * @param purchaseAccounts purchaseAccounts
	 */
	public void setPurchaseAccounts(final BigDecimal purchaseAccounts) {
		this.purchaseAccounts = purchaseAccounts;
	}
}
