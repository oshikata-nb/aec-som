 ...
 * @author tosco
 */
public final class MgrecipeProcedureDetailAction extends AbstractAction {
	...
	/** 処方プロシージャ工程詳細検索 ロジッククラス */
	private MgrecipeProcedureDetailLogic mgrecipeProcedureDetailLogic;
	 ...
	 * コンストラクタ.
	 */
	public MgrecipeProcedureDetailAction() {
		super();
	}
	...
	public ActionForward init(final ActionMapping mapping,
		...
		MgrecipeProcedureDetailForm detailForm = (MgrecipeProcedureDetailForm) form;
		...
		// 処方ヘッダ検索処理
		RecipeHeaderList header = mgrecipeProcedureDetailLogic.getHeader(
		...
		// 処方プロシージャ検索
		RecipeProcedureList bean = mgrecipeProcedureDetailLogic.getEntity(
	...
	private void setFormatData(final HttpServletRequest request,
			final MgrecipeProcedureDetailForm detailForm) {
	...
	public ActionForward update(final ActionMapping mapping,
		...
		MgrecipeProcedureDetailForm frm = (MgrecipeProcedureDetailForm) form;

		RecipeProcedureList bean = frm.getDetailBean();
		bean.setUpdatorCd(getLoginInfo(request).getTantoCd()); // 更新者ID
		// 画面内容をBeanへ設定
		setDetailFormData(frm, bean);

		// 工程マスタ存在チェック
		ActionMessages errors = mgrecipeProcedureDetailLogic
				.checkForUpdate(bean);
		if (!errors.isEmpty()) {
			// エラーがあった場合
		...
		// 処方プロシージャ更新処理
		mgrecipeProcedureDetailLogic.update(bean);
	 ...
	 */
	private MgrecipeProcedureDetailForm setCommonHeaderInfo(
			final HttpServletRequest request,
			final MgrecipeProcedureDetailForm form,
	...
	private void formatNum(final HttpServletRequest request,
			final RecipeProcedureList bean, final BigDecimal recipeUse) {
		CheckDigitUtilsLogic checker = CheckDigitUtil
				.getCheckDigitUtils(request);
		...
		} else if (MgrecipeConst.RECIPE_USE_PACKING.equals(recipeUse)) {
			// 包装の場合
	 ...
	 * @param bean 処方プロシージャBean
	 */
	private void setDetailFormData(final MgrecipeProcedureDetailForm frm,
			final RecipeProcedureList bean) {
	...
	/**
	 * 処方プロシージャ工程詳細検索 ロジッククラスを設定します。
	 * @param mgrecipeProcedureDetailLogic 処方プロシージャ工程詳細検索 ロジッククラス
	 */
	public void setMgrecipeProcedureDetailLogic(
			final MgrecipeProcedureDetailLogic mgrecipeProcedureDetailLogic) {
		this.mgrecipeProcedureDetailLogic = mgrecipeProcedureDetailLogic;
	}
