...
import com.asahikaseieng.app.comboboxes.SelectPaymentNoteDivision;
import com.asahikaseieng.dao.nonentity.payment.payment.AltPaymentBase;

/**
 * 支払入力用画面表示用1行クラス(カスタマイズ)
 * @author tosco
 */
public class PaymentCsmBean extends AltPaymentBase {

	/** serialVersionUID */
	private static final long serialVersionUID = 1L;

	/** 預金種別コンボボックス */
	private static final SelectAccountDivision combo = new SelectAccountDivision(
			false, true);

	/** 手形種別コンボボックス */
	private static final SelectPaymentNoteDivision noteCombo = new SelectPaymentNoteDivision(
			false, true);
	...
	/** 手形種別文字列 */
	private String noteDivisionString;
	...
	/** 読取手形種別 */
	private String viewPaymentNoteDivision;

	/**
	 * コンストラクタ
	 */
	public PaymentCsmBean() {
	}
	...
	public String getViewAccountDivision() {
		if (StringUtils.isNotEmpty(getAccountDivision())) {
			viewAccountDivision = getAccountDivision() + ":"
					+ combo.getLabel(getAccountDivision());
		} else {
			viewAccountDivision = "";
		}
	...
	// if (StringUtils.isNotEmpty(getAccountDivision())) {
	// this.viewAccountDivision = getAccountDivision() + ":" +
	// combo.getLabel(getAccountDivision());
	// }
	// }
	/**
	...
	/**
	 * 手数料計算フラグを設定します。
	 * @param feeFlag 手数料計算フラグ
	 */
	public void setFeeFlag(final int feeFlag) {
	...
	/**
	 * 手形必須区分を設定します。
	 * @param noteDiv 手形必須区分
	 */
	public void setNoteDiv(final int noteDiv) {
	 ...
	 * @return 手形種別文字列
	 */
	public String getNoteDivisionString() {
		return noteDivisionString;
	}

	/**
	 * 手形種別文字列を設定します。
	 * @param noteDivisionString 手形種別文字列
	 */
	public void setNoteDivisionString(final String noteDivisionString) {
		this.noteDivisionString = noteDivisionString;
	}
	 ...
	 * @return 読取手形種別
	 */
	public String getViewPaymentNoteDivision() {
		if (this.getNoteDivision() != null) {
			viewPaymentNoteDivision = this.getNoteDivision().toString() + ":"
					+ noteCombo.getLabel(this.getNoteDivision().toString());
		} else {
			viewPaymentNoteDivision = "";
		}
		return viewPaymentNoteDivision;
	}
