...
import com.asahikaseieng.dao.nonentity.grecipe.GrecipeCommonDetail;
import com.asahikaseieng.dao.nonentity.grecipe.GrecipeCommonDetailDao;
 ...
 * @author tosco
 */
public class GrecipeSpecificsLogicImpl implements GrecipeSpecificsLogic {

	/** 帳票・ラベルマスタ用Dao */
	private GrecipeLabelListDao grecipeLabelListDao;
	/** テンプレート保存先マスタ共通用Dao */
	private GrecipeCommonDetailDao grecipeCommonDetailDao;
	 ...
	 * コンストラクタ.原処方－詳細タブ
	 */
	public GrecipeSpecificsLogicImpl() {
	}
	 ...
	 */
	public List<GrecipeLabelList> getGrSearch(final String labelCd) {

		checkParams(labelCd);

		// 検索
		List<GrecipeLabelList> list = grecipeLabelListDao.getSearchList(labelCd);

		List<GrecipeLabelList> newList = new ArrayList<GrecipeLabelList>(numLine);
		for (int i = 0; i < numLine; i++) {
			GrecipeLabelList newBean = new GrecipeLabelList();
			// 新規登録時用－固定データ
			newBean.setLabelCd(labelCd);
			newBean.setLabelName("原処方");
			newBean.setCommonCd(GrecipeConst.COMMON_CD_GR + Integer.toString(i + 1));
	 ...
	 */
	public void regist(final GrecipeSpecificsForm frm, final String tantoCd) throws Exception {
		...
		for (GrecipeLabelList bean : frm.getSearchGrList()) {
			...
			try {
				...
				if (bean.getInputorCd() == null || bean.getInputorCd().equals("")) {
					...
					/* 追加処理 */
					updateNum = grecipeLabelListDao.insert(bean);
					if (updateNum != 1) {
						/* 排他エラー */
					...
					/* 更新処理 */
					updateNum = grecipeLabelListDao.update(bean);
					if (updateNum != 1) {
						/* 排他エラー */
						throw new OptimisticLockRuntimeException();
					}
	...
	public String getDownloadPath(final String commonId) {
		...
		GrecipeCommonDetail bean = grecipeCommonDetailDao.findByCommonCd(commonId);
	 ...
	 * @return アップロードファイル名
	 */
	private String upload(final GrecipeLabelList lblBean) {

		String outFile = null;

		GrecipeCommonDetail cmnBean = grecipeCommonDetailDao.findByCommonCd(lblBean.getCommonCd());
		if (cmnBean == null) {
			return outFile;
		}
	...
	/**
	 * 帳票・ラベルマスタ用Daoを設定します。
	 * @param grecipeLabelListDao 帳票・ラベルマスタ用Dao
	 */
	 ...
	 * @param grecipeCommonDetailDao テンプレート保存先マスタ共通用Dao
	 */
	public void setGrecipeCommonDetailDao(final GrecipeCommonDetailDao grecipeCommonDetailDao) {
		this.grecipeCommonDetailDao = grecipeCommonDetailDao;
	}
