...
/**
 * 出荷指図ヘッダーテーブルの出荷ステータス用コンボボックスクラス（出荷指図用）
 * @author tosco
 */
public class SelectShippingShippingStatus extends AbstractSelectComboBox {

	private static final String[] myValue = {"1", "2"};

	private static final String[] myLabel = {"出荷指図未確定", "出荷指図確定済"};

	/**
	 * コンストラクタ
	 */
	public SelectShippingShippingStatus() {
		setItem();
	}
	 ...
	 */
	public SelectShippingShippingStatus(final boolean zero, final boolean withNum) {
		super(zero, withNum);
		setItem();
	}
