...
import com.asahikaseieng.dao.nonentity.repPurchaseOrderDetail.RepPurchaseOrderDetail;
import com.asahikaseieng.dao.nonentity.repPurchaseOrderDetail.RepPurchaseOrderDetailDao;
import com.asahikaseieng.dao.nonentity.repPurchaseOrderHeader.RepPurchaseOrderHeader;
import com.asahikaseieng.dao.nonentity.repPurchaseOrderHeader.RepPurchaseOrderHeaderDao;
import com.asahikaseieng.poi.ReportExcelBuilder;
import com.asahikaseieng.servlet.FileDownloadInfo;
import com.asahikaseieng.utils.AecDateUtils;

/**
 * 発注書（注文書）ＥＸＣＥＬダウンロード用ファイル作成
 * @author t2712372
 */
public class PurchaseOrderListExcelDecoratorImpl implements
		PurchaseOrderListExcelDecorator {

	/** ビルダー */
	private ReportExcelBuilder builder = new ReportExcelBuilder();

	private RepPurchaseOrderHeaderDao repPurchaseOrderHeaderDao;

	private RepPurchaseOrderDetailDao repPurchaseOrderDetailDao;
	 ...
	 * コンストラクタ
	 */
	public PurchaseOrderListExcelDecoratorImpl() {
		super();
	 ...
	 * {@inheritDoc}
	 */
	public FileDownloadInfo createReport(final String[] orderSheetNo,
		...
		/* テンプレートをセット */
		builder.setWorkbookUrl("purchase_order");

		/* プリンタ設定シートをコピー */
		builder.setPrtSettingSheet();

		// 発注書ヘッダ取得用ヴューを検索
		List<RepPurchaseOrderHeader> headerList = repPurchaseOrderHeaderDao
				.getPurchaseOrderList(orderSheetNo, buySubcontractOrderNoArray);

		// 発注書明細取得用ヴューを検索
		List<RepPurchaseOrderDetail> detailList = repPurchaseOrderDetailDao
				.getPurchaseOrderDetailList(orderSheetNo,
					buySubcontractOrderNoArray);
	 ...
	 */
	private void setHeader(final List<RepPurchaseOrderHeader> headerList,
			final String tantoName, final Timestamp currentDate) {
		...
		for (RepPurchaseOrderHeader bean : headerList) {
			sCol = TEMP_START_COL;

			// 発注書No + 花王出力フラグ（集約のキー項目）
			builder.setCellValue(sRow, sCol++, bean.getOrderSheetNoKao());
			// 購買NO
			...
			// 仕入先コード
			builder.setCellValue(sRow, sCol++, bean.getSiCd());
			// 仕入先名
			builder.setCellValue(sRow, sCol++, bean.getSiName());
			// 仕入先担当者
			builder.setCellValue(sRow, sCol++, bean.getSiTantoName());
			// 仕入先TEL
			builder.setCellValue(sRow, sCol++, bean.getSiTelNo());
			// 仕入先FAX
			builder.setCellValue(sRow, sCol++, bean.getSiFaxNo());
			// 発注者名
			builder.setCellValue(sRow, sCol++, bean.getHName());
			// 発注者郵便
			builder.setCellValue(sRow, sCol++, bean.getHZipcodeNo());
			// 発注者住所
			builder.setCellValue(sRow, sCol++, bean.getHAddress());
			// 発注者TEL
			builder.setCellValue(sRow, sCol++, bean.getHTelNo());
			// 発注者FAX
			builder.setCellValue(sRow, sCol++, bean.getHFaxNo());
			// 発注担当者部署
			builder.setCellValue(sRow, sCol++, bean.getHTBusyo());
			// 発注担当者名
			builder.setCellValue(sRow, sCol++, bean.getHTantoName());
			// 下請法該当
			if (bean.getSubcontractLaw() == null) {
				builder.setCellValue(sRow, sCol++, "");
			} else {
				builder.setCellValue(sRow, sCol++, bean.getSubcontractLaw()
						.toString());
			}
			// 備考
			builder.setCellValue(sRow, sCol++, bean.getMasterRemark());
			// 納品場所会社名
			builder.setCellValue(sRow, sCol++, bean.getNohinName());
			// 納品場所住所
			builder.setCellValue(sRow, sCol++, bean.getNohinAddress());
			// 納品場所TEL
			builder.setCellValue(sRow, sCol++, bean.getNohinTelNo());
	 ...
	 * @param tantoName 担当者名
	 */
	private void setDetail(final List<RepPurchaseOrderDetail> detailList) {
		...
		for (RepPurchaseOrderDetail bean : detailList) {
			sCol = TEMP_START_COL;
			...
			// 届先名★
			builder.setCellValue(sRow, sCol++, bean.getTodokesaki());
	...
	/**
	 * 発注書詳細検索用Dao
	 * @param repPurchaseOrderDetailDao 発注書ヘッダ検索用Dao
	 */
	public final void setRepPurchaseOrderDetailDao(
			final RepPurchaseOrderDetailDao repPurchaseOrderDetailDao) {
		this.repPurchaseOrderDetailDao = repPurchaseOrderDetailDao;
	}

	/**
	 * 発注書ヘッダ検索用Dao
	 * @param repPurchaseOrderHeaderDao 発注書ヘッダ検索用Dao
	 */
	public final void setRepPurchaseOrderHeaderDao(
			final RepPurchaseOrderHeaderDao repPurchaseOrderHeaderDao) {
		this.repPurchaseOrderHeaderDao = repPurchaseOrderHeaderDao;
	}
