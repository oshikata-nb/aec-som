...
/**
 * 取引先マスタメンテ画面の請求書発行区分用コンボボックス
 * @author tosco
 */
public class SelectBillPublish extends AbstractSelectComboBox {

	private String[] myValue = {"1", "2"};

	private String[] myLabel = {"発行する", "発行しない"};

	/**
	 * コンストラクタ
	 */
	public SelectBillPublish() {
		setItem();
	}
	 ...
	 */
	public SelectBillPublish(final boolean zero, final boolean withNum) {
		super(zero, withNum);
		setItem();
	}
