...
/**
 * ExcelDecorator実装クラス.外注原材料投入実績一覧
 * @author t1344224
 */
public class MaterialRinputListExcelDecoratorImpl implements
		MaterialRinputListExcelDecorator {
	 ...
	 * コンストラクタ
	 */
	public MaterialRinputListExcelDecoratorImpl() {
	}
	...
	public FileDownloadInfo createReport(
		...
		/* テンプレートをセット */
		builder.setWorkbookUrl("materialrinputlist");
	 ...
	 */
	private void setReport(final MaterialRinputListConditionForReport condition) {
		short sRow = TEMP_START_ROW;
		short sCol = TEMP_START_COL;
		/* シートをセット */
		builder.setSheet("purchaselist");

		List<MaterialRinputListForReport> list = materialRinputListLogic
				.getReportList(condition);

		for (MaterialRinputListForReport bean : list) {
			sCol = TEMP_START_COL;
	...
	/**
	 * materialRinputListLogicを設定します。
	 * @param materialRinputListLogic materialRinputListLogic
	 */
