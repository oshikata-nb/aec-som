...
/**
 * ExcelDecorator実装クラス.受入・仕入一覧
 * @author tosco
 */
public class AcceptListListExcelDecoratorImpl implements
		AcceptListListExcelDecorator {
	 ...
	 * コンストラクタ
	 */
	public AcceptListListExcelDecoratorImpl() {
	}
	...
	public FileDownloadInfo createReport(
		...
		/* テンプレートをセット */
		builder.setWorkbookUrl("acceptlist");
	 ...
	 */
	private void setReport(final AcceptListConditionForReport condition) {
		short sRow = TEMP_START_ROW;
		short sCol = TEMP_START_COL;
		/* シートをセット */
		builder.setSheet("purchaselist");

		List<AcceptListForReport> list = acceptListLogic
				.getReportList(condition);
		for (AcceptListForReport bean : list) {
			sCol = TEMP_START_COL;
	...
	/**
	 * acceptListLogicを設定します。
	 * @param acceptListLogic acceptListLogic
	 */
