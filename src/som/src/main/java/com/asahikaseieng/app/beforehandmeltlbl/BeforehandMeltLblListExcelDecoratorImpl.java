...
import com.asahikaseieng.dao.nonentity.repLabelYobiyokai.RepLabelYobiyokai;
import com.asahikaseieng.dao.nonentity.repLabelYobiyokai.RepLabelYobiyokaiDao;
import com.asahikaseieng.poi.ReportExcelBuilder;
import com.asahikaseieng.servlet.FileDownloadInfo;
import com.asahikaseieng.utils.AecDateUtils;

/**
 * 予備溶解ラベルＥＸＣＥＬダウンロード用ファイル作成
 * @author t2712372
 */
public class BeforehandMeltLblListExcelDecoratorImpl implements
		BeforehandMeltLblListExcelDecorator {

	/** ビルダー */
	private ReportExcelBuilder builder = new ReportExcelBuilder();

	private RepLabelYobiyokaiDao repLabelYobiyokaiDao;
	 ...
	 * コンストラクタ
	 */
	public BeforehandMeltLblListExcelDecoratorImpl() {
		super();
	...
	public FileDownloadInfo createReport(
		...
		/* テンプレートをセット */
		builder.setWorkbookUrl("label_yobiyokai");

		/* プリンタ設定シートをコピー */
		builder.setPrtSettingSheet();

		// 製品ラベル取得用ヴューを検索
		List<RepLabelYobiyokai> headerList = repLabelYobiyokaiDao
				.getYobiyokaiList(directionNoArray);
	 ...
	 */
	private void setHeader(final List<RepLabelYobiyokai> headerList,
		...
		for (RepLabelYobiyokai bean : headerList) {
			sCol = TEMP_START_COL;

			// ラベル枚数（1固定）
			builder.setCellValue(sRow, sCol++, "1");
			// ラベル種別（未使用）
			...
			// 工場名★
			builder.setCellValue(sRow, sCol++, bean.getProductionLineName());
			// 予備溶解番号★
			if (bean.getYobiyokaino() == null) {
				builder.setCellValue(sRow, sCol++, "");
			} else {
				builder.setCellValue(sRow, sCol++, bean.getYobiyokaino()
						.toString());
			}
			// 予備溶解バーコード★
			builder.setCellValue(sRow, sCol++, bean.getYobiyokaibcr());
	...
	/**
	 * 予備溶解ラベル検索用Dao
	 * @param repLabelYobiyokaiDao 予備溶解ラベル検索用Dao
	 */
	public void setRepLabelYobiyokaiDao(
			final RepLabelYobiyokaiDao repLabelYobiyokaiDao) {
		this.repLabelYobiyokaiDao = repLabelYobiyokaiDao;
	}
}
