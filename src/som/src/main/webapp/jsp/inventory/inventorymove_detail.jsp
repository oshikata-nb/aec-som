...
<script language="JavaScript" type="text/javascript">
	...
	Event.observe(window, 'load', function() {
		...
		defineAsRequiredField("inLocationCd");
		defineAsRequiredField("ryCd");

		<%-- 数値型フィールド定義 --%>
		if($("typeDivision").value == '0'
		|| $("typeDivision").value == '4'
		|| $("typeDivision").value == '5'
		|| $("typeDivision").value == '6'
		|| $("typeDivision").value == '7'
		|| $("typeDivision").value == '9'){
		...
		new Ajax.Autocompleter(
			...
			{
				paramName : "code",
				afterUpdateElement : setInLocationLabel
			}
		);
		...
		<%-- 検索後入力された場合の不整合をただす --%>
		Event.observe('inLocationCd', 'keypress', clearLabel.bindAsEventListener($('inLocationName')), false);
	...
	<%-- ajax --%>
	function setInLocationLabel(text, li) {
		<%-- ここでinnerTextをラベルとして表示 --%>
		$("inLocationName").value = li.getElementsByTagName('span')[0].innerText;
		refreshLabel();
		setDirtyFlg();
	}
...
<nested:form action="/InventoryMoveDetail" method="post" onsubmit="return false;" styleId="mainForm">
	<%@ include file="/jsp/common/postjavascript.jsf"%>

	<nested:hidden property="op" styleId="op" />
	<nested:hidden property="cursor" styleId="cursor" />
	<nested:hidden property="typeDivision" styleId="typeDivision" />
						...
						<td>
							...
							<table width="100%" cellspacing="2" cellpadding="2" border="0">
								...
								<tr>
									...
									<td>
										<nested:write property="outLocationCd" />
									</td>

									<td colspan="3">
										<nested:write property="outLocationName" />
									</td>
								</tr>
								...
								<tr>
									...
									<nested:equal property="typeDivision" value="0">
										<td class="fcTitleDetail fb bcTitleDetail">荷姿数</td>
									</nested:equal>

									<nested:equal property="typeDivision" value="4">
										<td class="fcTitleDetail fb bcTitleDetail">荷姿数</td>
									</nested:equal>

									<nested:equal property="typeDivision" value="5">
										<td class="fcTitleDetail fb bcTitleDetail">荷姿数</td>
									</nested:equal>

									<nested:equal property="typeDivision" value="6">
										<td class="fcTitleDetail fb bcTitleDetail">荷姿数</td>
									</nested:equal>

									<nested:equal property="typeDivision" value="7">
										<td class="fcTitleDetail fb bcTitleDetail">荷姿数</td>
									</nested:equal>

									<nested:equal property="typeDivision" value="9">
										<td class="fcTitleDetail fb bcTitleDetail">荷姿数</td>
									</nested:equal>
								...
								<tr>
									<td class="fcTitleDetail fb bcTitleDetail">端数在庫数</td>
									<td>
								...
								<tr>
									...
									<td>
										<nested:text property="inLocationCd" maxlength="20" size="20" styleId="inLocationCd" onchange="setDirtyFlg();" />
										<div id="autocomplete_choices" class="autocomplete"></div>
									</td>

									<td colspan="3" width="450">
										<div id="lblInLocationName">
											<nested:write property="inLocationName" />
										</div>
										<nested:hidden property="inLocationName" styleId="inLocationName"/>
									</td>
								</tr>
