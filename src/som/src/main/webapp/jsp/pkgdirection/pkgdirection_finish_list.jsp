...
<script language="JavaScript" type="text/javascript">
	...
	Event.observe(window, 'load', function() {
		...
		for( var i = 0 ; i < count ; i++){
			...
			defineAsNumberField("strFillQty" + i);
		}
	...
	<%-- 充填予定数量のフォーマット --%>
	function formatFillQty(){
		var index = $F("line");
		var value = $F("strFillQty" + index);
		<%-- 小数点桁数・端数区分で数値を再フォーマット --%>
	    var decPoint = $('decimalPoint' + index).value;
	    var round = $('roundDivision' + index).value;
		if ((value != null) && (value != "")) {
			$("strFillQty" + index).value = digitStringFormat(decPoint,round,value);
		}
	}
...
<nested:form action="/PkgDirectionFinishList" method="post" onsubmit="return false;" styleId="mainForm">
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tr>
					...
					<table border="0" cellpadding="0" cellspacing="0" width="100%">
						...
						<tr>
							<td class="fcTitle fb">包装指図出力仕上</td>
							<td>
											...
											<nested:notEmpty property="searchFinishList">
												<table cellspacing="2" cellpadding="1" width="100%" border="0" id="tblList">
													<tr class="bcTitleList fb fcTitleList">
														...
														<td>品目名称</td>
														<td>区分</td>
														<td width="100">生産予定数量</td>
														<td width="100">充填予定数量</td>
													...
													<nested:iterate id="searchFinishList" property="searchFinishList" indexId="index">
														...
														<nested:equal name="index" value="0" >
															...
															<%-- 生産予定数量 --%>
															<td>
																<nested:text property="strQty" maxlength="22" size="10" styleId='<%= "strQty" + pageContext.findAttribute("index").toString() %>'
																 style="ime-mode:disabled" styleClass="alignRight" onchange="setDirtyFlg();" onblur="<%="setLine(" + pageContext.findAttribute("index").toString() + ");formatQty();"%>" />
															</td>
															<%-- 充填予定数量 --%>
															<td>
																<nested:text property="strFillQty" maxlength="22" size="10" styleId='<%= "strFillQty" + pageContext.findAttribute("index").toString() %>'
																 style="ime-mode:disabled" styleClass="alignRight" onchange="setDirtyFlg();" onblur="<%="setLine(" + pageContext.findAttribute("index").toString() + ");formatFillQty();"%>" />
															</td>
															<%-- ロット番号 --%>
															<td>
														...
														<nested:notEqual name="index" value="0" >
															...
															<nested:empty property="strSeq">
																<td>
																	<nested:select property="strStepNo" style="margin: 0;padding: 0" styleId="strStepNo" onchange="setDirtyFlg();">
																		<nested:optionsCollection name="pkgDirectionFinishListForm" property="stepNoCombo" label="labales" value="values" />
																	</nested:select>
																</td>
															</nested:empty>
															...
															<td>
																<nested:text property="strFillQty" maxlength="22" size="10" styleId='<%= "strFillQty" + pageContext.findAttribute("index").toString() %>'
																	onblur="<%="setLine(" + pageContext.findAttribute("index").toString() + ");formatFillQty();"%>" style="ime-mode:disabled" styleClass="alignRight" onchange="setDirtyFlg();" />
															</td>
															<%-- ロット番号 --%>
															<td>
																<nested:text property="lotNo" maxlength="20" size="10" styleId='<%= "lotNo" + pageContext.findAttribute("index").toString() %>'
