...
<script language="JavaScript" type="text/javascript">
	...
	function selectClose(index){
		window.opener.setOrderDeliveryValues(true
			, $F("deliveryCd" + index)
			, $F("searchKana" + index)
			, $F("address" + index)
			, $F("telNo" + index)
			, $F("fareClaimExistence" + index)
			, $F("eigyoTantoCd" + index)
			, $F("eigyoTantoName" + index)
			, $F("strLeadTime" + index)
			, $F("deliveryTime" + index)
			, $F("carryCd" + index)
		);
	...
	function selectCancel(){
		window.opener.setOrderDeliveryValues(false, "", "", "", "", "", "", "", "", "", "");
...
<nested:form action="/OrderDeliverySearch" method="post" onsubmit="return false;" styleId="mainForm">
									...
									<td>
										<table cellspacing="2" cellpadding="0" border="0">
											<tr>
												...
												<td>
													<%
														pageContext.setAttribute("selectOrderDeliveryDivision",
														new com.asahikaseieng.app.comboboxes.SelectOrderDeliveryDivision(false, false));
													%>
													<nested:select property="srhDivision" styleId="srhDivision"  onchange="setDeliveryDivision();">
														<nested:options name="selectOrderDeliveryDivision" property="values" labelName="selectOrderDeliveryDivision" labelProperty="labels" />
													</nested:select>
												</td>
											</tr>
										</table>
						...
						<td><%-- 明細部 --%>
							<table cellspacing="2" cellpadding="2" width="100%">
								...
								<app:modulo numerator='<%=pageContext.findAttribute("index").toString()%>' denominator="2" zero="true">
									...
									<%-- 運賃請求有無フラグ --%>
									<nested:hidden property="fareClaimExistence" styleId="<%="fareClaimExistence" + pageContext.findAttribute("index").toString() %>"/>

									<%-- 営業担当者コード --%>
									<nested:hidden property="eigyoTantoCd" styleId="<%="eigyoTantoCd" + pageContext.findAttribute("index").toString() %>"/>

									<%-- 営業担当者名称 --%>
									<nested:hidden property="eigyoTantoName" styleId="<%="eigyoTantoName" + pageContext.findAttribute("index").toString() %>"/>

									<%-- リードタイム --%>
									<nested:hidden property="strLeadTime" styleId="<%="strLeadTime" + pageContext.findAttribute("index").toString() %>"/>

									<%-- 納入時刻 --%>
									<nested:hidden property="deliveryTime" styleId="<%="deliveryTime" + pageContext.findAttribute("index").toString() %>"/>
