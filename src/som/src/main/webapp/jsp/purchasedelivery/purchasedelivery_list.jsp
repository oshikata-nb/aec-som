...
<script language="JavaScript" type="text/javascript">
	...
	Event.observe(window, 'load', function() {
		...
		<%-- フォーム設定の初期化 --%>
		<logic:equal name="purchaseDeliveryListForm" property="searchList" value="false">
			initializeFormState();
		</logic:equal>
	...
	function toSumDetail(url) {
	    if (!g_lock) {
	        if (continueConfirm()) {
	        	var srhOrderSheetNo = $F("srhOrderSheetNo");
	          	location.href = url + srhOrderSheetNo;
	  			g_lock = true;
	    	}
      	}
	}
...
<nested:form action="/PurchaseDeliveryList" method="post" onsubmit="return false;" styleId="mainForm">
			...
			<table border="0" cellpadding="0" cellspacing="0" width="100%">
				<tr>
					...
					<table border="0" cellpadding="0" cellspacing="0" width="100%">
						...
						<tr>
							<td class="fcTitle fb" width="250">納期回答</td>
							<td>
							...
							<table border="0" cellspacing="2" cellpadding="1">
								<tr>
									...
									<td>
										<%
											...
											new com.asahikaseieng.app.comboboxes.SelectPurchaseStatusDelivery(true, true));
										%>
								...
								<tr>
									...
									<td>
										...
										<span  id="cssButton">
											<a href="#"
												 onclick="<%="toSumDetail('" + request.getContextPath() + "/PurchaseDeliverySumDetail.do?op=jump&orderSheetNo='); return false;"%>"
												 class="cssButton">
												&nbsp;&nbsp;まとめ入力&nbsp;&nbsp;
											</a>
										</span>
							...
							<tr>
								...
								<table cellspacing="2" cellpadding="1" id="tblList">
									<tr class="bcTitleList fb fcTitleList">
										<td width="80">発注書No</td>
										<td width="80">発注日</td>
										<td>仕入先</td>
										<td width="90">全オーダー<br>件数</td>
										<td width="100">発注書発行済<br>件数</td>
										<td width="90">納期調整中<br>件数</td>
										<td width="90">納期確定<br>件数</td>
										<td width="90">入荷・受入済<br>件数</td>
									</tr>
									<nested:iterate id="searchList" property="searchList"
										...
										<nested:define id="osno" property="orderSheetNo" />

												<%-- 発注書No --%>
												<td>
													<html:link href="#" onclick='<%="toDetail('" + request.getContextPath() + "/PurchaseDeliverySumDetail.do?op=init&orderSheetNo=" + pageContext.findAttribute("osno").toString() + "'); return false;"%>'>
												...
												<%-- 全オーダー件数 --%>
												<td class="alignRight">
													<nested:write property="strOrderCount" />
												</td>
												<%-- 発注書発行済件数 --%>
												<td class="alignRight">
													<nested:write property="strIssuedCount" />
												</td>
												<%-- 発注書発行済件数 --%>
												<td class="alignRight">
													<nested:write property="strAdjustCount" />
												</td>
												<%-- 納期確定件数 --%>
												<td class="alignRight">
													<nested:write property="strFixedCount" />
												</td>
												<%-- 入荷・受入済件数 --%>
												<td class="alignRight">
													<nested:write property="strArrivedAcceptedCount" />
												</td>
											</tr>
									</nested:iterate>
								</table>
