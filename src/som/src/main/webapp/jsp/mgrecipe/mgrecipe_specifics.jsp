...
<script language="JavaScript" type="text/javascript">
	...
	Event.observe(window, 'load', function() {
		...
		for(var i = 0 ; i < 4 ; i++){
			...
			$('lblMrFileLink' + i).update("ファイルリンク" + index);
		}
	...
	function setMDiv(index) {
		var lineNo = index + 1;
		$("downloadDiv").value = "MRECIPE_DETAIL" + lineNo;
	}
...
<nested:form action="/MgrecipeSpecifics" method="post" enctype="multipart/form-data" onsubmit="return false;" styleId="mainForm">

	<%@ include file="/jsp/common/postjavascript.jsf"%>
	
	<nested:hidden property="originalRecipeId"></nested:hidden>
	<nested:hidden property="downloadDiv"></nested:hidden>
	<nested:hidden property="oriRecFlg"></nested:hidden>
									...
									<span id="slidetabs" style="clear: left;">
										<table border="0" cellspacing="2" cellpadding="2" width="100%">
											...
											<nested:iterate id="searchGrList" property="searchGrList" indexId="index">
												...
												<nested:equal name="mgrecipeSpecificsForm" property="oriRecFlg" value="0">
													<tr>
												...
												<nested:equal name="mgrecipeSpecificsForm" property="oriRecFlg" value="1">
													<tr>
														<td class="fcTitleDetail fb bcTitleDetail" width="130">製品仕様書
															<div id="<%="lblGrFileLink" + pageContext.findAttribute("index").toString() %>">
															</div>
														...
														<td>
															<nested:file property="uploadFile" size="20" onchange="setDirtyFlg();" disabled="true" />
															<div id="cssButton">
															<nested:text property="dispLabelPath" styleId="dispLabelPath" readonly="true" size="40" onchange="setDirtyFlg();" disabled="true" />
																<a href="#" onclick="return false;" class="cssButton" disabled>
																&nbsp;DOWN&nbsp;
																</a>
															</div>
														</td>
											...
											<nested:iterate id="searchMrList" property="searchMrList" indexId="index">

												<nested:hidden property="commonCd" styleId="<%="commonCd" + pageContext.findAttribute("index").toString() %>" />

												<tr>
													<td class="fcTitleDetail fb bcTitleDetail" width="130" >
														<div id="<%="lblMrFileLink" + pageContext.findAttribute("index").toString() %>">
														</div>
													</td>
													<td>
														<nested:file property="uploadFile" size="20" onchange="setDirtyFlg();" />
														<div id="cssButton">
														<nested:text property="dispLabelPath" styleId="dispLabelPath" readonly="true" size="40" onchange="setDirtyFlg();" />
															<a href="#" class="cssButton"
																 onclick="<%="if (!(putDirtyConfirm())) {return false;}else{setMDiv(" + pageContext.findAttribute("index").toString() + ");return form_submit('op', 'filedownload'); return false;}"%>">
															&nbsp;DOWN&nbsp;
															</a>
														</div>
													</td>
