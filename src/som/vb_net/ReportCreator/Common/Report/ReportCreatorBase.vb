...
#Region "変数"
    ...
    ''' </summary>
    ''' <remarks></remarks>
    Private m_dbAcs As New DatabaseAccess
...
#Region "DB関連"
    ...
    Private Function DbOpen() As Boolean
        ...
        Try
            'DB接続
            Dim dbInfo As New DatabaseAccess.DataBaseAccessInfo
            dbInfo.SetDataBaseAccessInfo(ConfigurationManager.AppSettings("DbType") _
                                          , ConfigurationManager.AppSettings("InitialCatalog") _
                                          , ConfigurationManager.AppSettings("UserName") _
                                          , ConfigurationManager.AppSettings("PassWord") _
                                          , ConfigurationManager.AppSettings("Servicename"))
            Call m_dbAcs.DBConnect(dbInfo)

            Return True
        Catch ex As Exception
            'エラー出力
            m_log.Error("[DbOpen]", ex)
            Return False
        End Try
    ...
    Private Function DbClose() As Boolean
        ...
        Try
            'DB切断
            Call m_dbAcs.DBDisconnect()

            Return True
        Catch ex As Exception
            'エラー出力
            m_log.Error("[DbClose]", ex)
            Return False
        End Try
    ...
    ''' <returns></returns>
    ''' <remarks></remarks>
    Protected Function GetData(sql As String) As Data.DataTable

        Try
            Dim ds As DataSet = m_dbAcs.GetDataSet(sql)
            If ds Is Nothing _
                OrElse ds.Tables Is Nothing _
                OrElse ds.Tables.Count = 0 Then
                Return Nothing
            End If
...
#Region "Excel関連"
            ...
            Try
                ...
                Try
                    ...
                    Try
                        ...
                        'シートの編集
                        ' If Not EditSheet(exlSheet) Then
                        'Return PROC_RESULT.ロジックエラー
                        'End If

                        ' ブックの編集（必要ならばオーバーライドして使用）
                        'If Not EditBook(exlBook) Then
                        '   Return PROC_RESULT.ロジックエラー
                        'End If
                        ...
                        Select Case m_fileType
                            ...
                            Case Else
                                ...
                                'exlBook.Close(SaveChanges:=True, Filename:=m_outputFile)   '不要
    ...
    ''' <remarks></remarks>
    Protected MustOverride Function EditSheet(exlSheet As Excel.Worksheet) As Boolean
    ...
    ''' <remarks></remarks>
    Protected Overridable Function EditBook(exlBook As Excel.Workbook) As Boolean
        Return True
    End Function
...
#Region "FAX関連"
    ...
    ''' <remarks></remarks>
    Protected Sub SetFaxInfo(faxNo As String)

        m_faxManager.FaxNo = faxNo
    ...
    Protected Sub SetTantoNameInfo(tantoName As String)

        m_faxManager.Name = tantoName
